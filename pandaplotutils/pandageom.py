...
class PandaDecorator(object):
    """
    use class to preload files and generate various models
    the models are for decorating purpose, they are not collision models and cannot be used to do collision detection
    """
    ...
    def genArrow(self, length, thickness = 1.5, plotname = "arrow"):
        ...
        arrow.setTransparency(TransparencyAttrib.MDual)
    ...
    def genSphere(self, pos = [0,0,0], radius = 0.05, plotname = "sphere"):
        """
    ...
    def plotArrow(self, nodepath, spos = None, epos = None, length = None, thickness = 1.5, rgba=None):
        ...
        if epos is None:
            epos = np.array([0,0,1])
        if length is None:
            length = np.linalg.norm(np.array(epos)-np.array(spos))
        if rgba is None:
            rgba = np.array([1,1,1,1])
    ...
    def plotDumbbell(self, nodepath, spos=None, epos=None, length=None, thickness=1.5, rgba=None, plotname="dumbbell", headscale=2):
        """
    ...
    def plotBox(self, nodepath, pos = None, x = 1.0, y = 1.0, z = 1.0, rgba=None):
        ...
        boxnp.setTransparency(TransparencyAttrib.MDual)
    ...
    def plotLinesegs(self, nodepath, verts, thickness = 1.5, rgba=None, plotname="linesegs", headscale = 1):
        ...
        for i in range(len(verts)-1):
            ...
            dumbbell.setColor(rgba[0], rgba[1], rgba[2], rgba[3])
            dumbbell.setTransparency(TransparencyAttrib.MDual)
        # lookAt points y+ to epos, use the following command to point x+ to epos
        # http://stackoverflow.com/questions/15126492/panda3d-how-to-rotate-object-so-that-its-x-axis-points-to-a-location-in-space
        # arrow.setHpr(arrow, Vec3(0,0,90))
    ...
    def plotSphere(self, nodepath, pos=None, radius=None, rgba=None, plotname="sphere"):
        ...
        spherend.setTransparency(TransparencyAttrib.MDual)
    ...
    def plotStick(self, nodepath, spos = None, epos = None, length = None, thickness = 1.5, rgba=None, plotname="dumbbell"):
        ...
        stick.setTransparency(TransparencyAttrib.MDual)
        stick.reparentTo(nodepath)
        return stick

    def plotFrame(self, nodepath, pos=np.array([0,0,0]), rot=np.eye(3), length=300, thickness = 10 ,rgba = None, alpha=None):
        """
        ...
        :param pos, rot: numpy 1-by-3 pos and 3-by-3 rot
        :return: null
        ...
        if rgba is None:
            if alpha is None:
                self.plotArrow(nodepath, pos, pos+rot[:3,0], length=length, thickness=thickness, rgba=[1,0,0,1])
                self.plotArrow(nodepath, pos, pos+rot[:3,1], length=length, thickness=thickness, rgba=[0,1,0,1])
                self.plotArrow(nodepath, pos, pos+rot[:3,2], length=length, thickness=thickness, rgba=[0,0,1,1])
            else:
                self.plotArrow(nodepath, pos, pos+rot[:3,0], length=length, thickness=thickness, rgba=[1,0,0,alpha])
                self.plotArrow(nodepath, pos, pos+rot[:3,1], length=length, thickness=thickness, rgba=[0,1,0,alpha])
                self.plotArrow(nodepath, pos, pos+rot[:3,2], length=length, thickness=thickness, rgba=[0,0,1,alpha])
        else:
            self.plotArrow(nodepath, pos, pos+rot[:3,0], length=length, thickness=thickness, rgba=rgba)
            self.plotArrow(nodepath, pos, pos+rot[:3,1], length=length, thickness=thickness, rgba=rgba)
            self.plotArrow(nodepath, pos, pos+rot[:3,2], length=length, thickness=thickness, rgba=rgba)

    def plotAxis(self, nodepath, spos=Vec3(0,0,0), srot=Mat3.identMat(), length=300, thickness = 10 ,rgba = None, alpha=None):
        """
        plot an axis to the scene, using self-defined arrows
        note: pos is in the coordiante sys of nodepath
        ...
        :param spos, srot: could be either numpy or panda
        :return: null
        ...
        y = np.array([0,1,0])
        z = np.array([0,0,1])
        if isinstance(spos, np.ndarray) and isinstance(srot, np.ndarray):
            x = srot[:,0]
            y = srot[:,1]
            z = srot[:,2]
        elif isinstance(srot, LMatrix3f):
            x = srot.getRow(0)
            y = srot.getRow(1)
            z = srot.getRow(2)
        else:
            print("Rotmat3 must be either numpy array or panda mat3.")
            raise ValueError()

        if rgba is None:
            if alpha is None:
                self.plotArrow(nodepath, spos, spos+x, length=length, thickness=thickness, rgba=[1,0,0,1])
                self.plotArrow(nodepath, spos, spos+y, length=length, thickness=thickness, rgba=[0,1,0,1])
                self.plotArrow(nodepath, spos, spos+z, length=length, thickness=thickness, rgba=[0,0,1,1])
            else:
                self.plotArrow(nodepath, spos, spos+x, length=length, thickness=thickness, rgba=[1,0,0,alpha])
                self.plotArrow(nodepath, spos, spos+y, length=length, thickness=thickness, rgba=[0,1,0,alpha])
                self.plotArrow(nodepath, spos, spos+z, length=length, thickness=thickness, rgba=[0,0,1,alpha])
        else:
            self.plotArrow(nodepath, spos, spos+x, length=length, thickness=thickness, rgba=rgba)
            self.plotArrow(nodepath, spos, spos+y, length=length, thickness=thickness, rgba=rgba)
            self.plotArrow(nodepath, spos, spos+z, length=length, thickness=thickness, rgba=rgba)
    ...
    def plotTorus(self, nodepath, axis = None, portion = 1.0, center = None, radius = 5.0,
                      thickness = 1.5, rgba = None, discretization = 24, plotname="torus"):
        """
        Draw a torus
        Especially used for visualization of circles on a sphere
        the torus starts from (axis cross [1,0,0])*radius+center
        if axis cross [1,0,0] equals [0,0,0], the torus starts from (axis cross [0,1,0])*radius+center
        the portion of the torus indicates if it is an arc or closed
        ...
        torusnp = NodePath(plotname)
        discretizedangle = 360.0/discretization
        ndist = int(portion*discretization)+1
        lastpos = startingpos
        if abs(ndist)>1:
            for i in range(1*np.sign(ndist), ndist, 1*np.sign(ndist)):
                nxtpos = center+np.dot(rm.rodrigues(axis, i*discretizedangle), startingaxis.T).T*radius
                self.plotStick(torusnp, spos = lastpos, epos = nxtpos, thickness = thickness, rgba = rgba, plotname = "circarrseg")
                lastpos = nxtpos
        torusnp.reparentTo(nodepath)
...
def packpandageompnts(vertices, colors=np.array([]), name=''):
    """
    package the vertices and triangles into a panda3d geom
    ...
    vertices:
        a n-by-3 nparray, each row is a vertex
    colors:
        a n-by-4 nparray, each row is a rgba or a 1-by-4 array specifying a unified color
    name:
        not as important
    ...
    for i,vert in enumerate(vertices):
        ...
        if len(colors) == 0:
            ...
            colorwritter.addData4f(colors[0], colors[1], colors[2], colors[3])
        else:
...
def facetboundary(objtrimesh, facet, facetcenter, facetnormal):
    ...
    for i, faceidx in enumerate(facet):
        ...
        vert0p = rm.transformmat4(facetnpmat4, vert0)
        vert1p = rm.transformmat4(facetnpmat4, vert1)
        vert2p = rm.transformmat4(facetnpmat4, vert2)
    ...
    for vert2d in verts2d:
        vert3d = rm.transformmat4(rm.homoinverse(facetnpmat4), np.array([vert2d[0], vert2d[1], 0]))[:3]
        verts3d.append(vert3d)

    return [verts3d, verts2d, facetnpmat4]

def genobjmeshnp(objpath, objname = 'obj', color = Vec4(1,0,0,1)):
    """
    gen objmnp
    ...
    objmnp.setTransparency(TransparencyAttrib.MDual)

    return objmnp

def genpointcloudnp(verts, colors = [], pntsize = 1):
    """
    gen objmnp
...
def gensurfacenp(verts, tris, color = Vec4(1,0,0,1)):
    ...
    surfacemnp.setTransparency(TransparencyAttrib.MDual)
    surfacemnp.setTwoSided(True)

    return surfacemnp

def genpolygonnp(verts, colors = [], thickness = 2.0):
    """
    gen objmnp
...
def genlinesegnp(verts, colors = [], thickness = 2.0):
    """
    gen objmnp
...
def o3dtrimeshtonp(o3dtrimesh, name = "auto"):
    """
    cvt open3d trimesh models to panda models
    ...
    return packpandanp_fn(o3dtrimesh.vertices, o3dtrimesh.triangle_normals, o3dtrimesh.triangles)
...
if __name__=="__main__":
    ...
    base = pandactrl.World(camp=[0,0,3000], lookatpos=[0, 0, 0])
    ...
    pntsnp = genpointcloudnp(verts, pntsize = 10)
